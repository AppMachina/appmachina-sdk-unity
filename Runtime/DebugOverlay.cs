...
//
// Toggle via AppMachinaSDK.ShowDebugOverlay() / AppMachinaSDK.HideDebugOverlay().
...
namespace AppMachina.Unity
{
    ...
    ///
    /// This component is added to the [AppMachina] hidden GameObject and is
    /// toggled via <see cref="AppMachinaSDK.ShowDebugOverlay"/> and
    /// <see cref="AppMachinaSDK.HideDebugOverlay"/>.
    /// </summary>
    internal class DebugOverlay : MonoBehaviour
    {
        ...
        // ── Static Event Recording (called by AppMachinaSDK.Track) ────────────
        ...
        private void DrawHeader()
        {
            ...
            GUILayout.Label("AppMachina SDK Debug", _headerStyle);
        ...
        {
            DrawRow("SDK Version", $"unity/{AppMachinaSDK.SdkVersion}");
        ...
        {
            _sdkInitialized = AppMachinaSDK.IsInitialized;

            if (_sdkInitialized)
            {
                string sessionId = AppMachinaSDK.SessionId;
                _sessionId = TruncateId(sessionId);
                _queueDepth = AppMachinaSDK.QueueDepth;
            }
            else
            {
                _sessionId = "--";
            ...
            _userId = string.IsNullOrEmpty(AppMachinaSDK.UserId) ? "(anonymous)" : AppMachinaSDK.UserId;
            _installId = TruncateId(InstallIdProvider.GetOrCreate());
            _lastFlush = _lastFlushInfo ?? "never";

            // Read config state via internal accessors on the AppMachinaSDK facade
            _environment = AppMachinaSDK.Environment ?? "--";
            _appId = AppMachinaSDK.AppId ?? "--";

            // Read consent from the Rust core's remote config if available
            string remoteConfig = AppMachinaSDK.RemoteConfig;
