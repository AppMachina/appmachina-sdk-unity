...
namespace AppMachina.Unity
{
    ...
    public static class SKANModule
    {
        ...
        private static extern bool appmachina_skan_is_supported();

        [DllImport("__Internal")]
        private static extern IntPtr appmachina_skan_get_version();

        [DllImport("__Internal")]
        private static extern void appmachina_skan_register();

        [DllImport("__Internal")]
        private static extern void appmachina_skan_update_conversion_value(int fineValue);

        [DllImport("__Internal")]
        private static extern void appmachina_skan_update_postback(
        ...
        public static bool IsSupported
        {
            ...
            {
#if UNITY_IOS && !UNITY_EDITOR
                return appmachina_skan_is_supported();
#else
                return false;
#endif
            }
        ...
        {
#if UNITY_IOS && !UNITY_EDITOR
            IntPtr ptr = appmachina_skan_get_version();
            if (ptr == IntPtr.Zero)
                return "unsupported";
        ...
        public static void UpdateConversionValue(int fineValue)
        {
            if (fineValue < 0 || fineValue > 63)
            {
                UnityEngine.Debug.LogWarning(
                    $"[AppMachina] SKANModule.UpdateConversionValue: fineValue {fineValue} is outside valid range 0-63. Clamping.");
                fineValue = Math.Clamp(fineValue, 0, 63);
            }

#if UNITY_IOS && !UNITY_EDITOR
            appmachina_skan_update_conversion_value(fineValue);
#endif
        }
            ...
            int fineValue, SKANCoarseValue coarseValue, bool lockWindow)
        {
            ...
            {
                UnityEngine.Debug.LogWarning(
                    $"[AppMachina] SKANModule.UpdatePostbackConversionValue: fineValue {fineValue} is outside valid range 0-63. Clamping.");
                fineValue = Math.Clamp(fineValue, 0, 63);
            }

#if UNITY_IOS && !UNITY_EDITOR
            string coarseStr = CoarseValueToString(coarseValue);
            appmachina_skan_update_postback(fineValue, coarseStr, lockWindow);
#endif
        }
        ...
        public static void SetPreset(string preset)
        {
            ...
            {
                AppMachinaLogger.Warn($"SKAN: unknown preset '{preset}'");
                return;
            }
            _rules = rules;
        ...
        ///
        /// Call this after every <see cref="AppMachinaSDK.Track"/> call when SKAN rules are
        ...
        public static void ConfigureFromRemoteConfig(string configJson)
        {
            ...
            {
                AppMachinaLogger.Warn($"SKAN auto-config: failed to parse config JSON: {e.Message}");
                return;
            }
            ...
            if (skanObj.ContainsKey("preset") && skanObj["preset"] is string preset)
            {
                ...
                if (presetRules != null && presetRules.Count > 0)
                {
                    ...
                    AppMachinaLogger.Log($"SKAN auto-configured from remote config: preset={presetLower}");
                }
                else
                {
                    AppMachinaLogger.Warn($"SKAN auto-config: unknown preset '{preset}'");
                }
                return;
            }
