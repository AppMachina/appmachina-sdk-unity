...
{
    /// <summary>
    /// Android-specific module for the AppMachina Unity SDK.
    ///
    ...
    {
        private const string Tag = "AppMachinaSDK";
            ...
            System.Threading.Tasks.Task.Run(() =>
            {
                try
                {
                        ...
                        "getAdvertisingIdInfo", appContext))
                    {
                        if (adInfo == null)
                        {
                            Debug.LogWarning("[AppMachinaSDK] GAID fetch failed: Google Play Services unavailable or returned null");
                            callback?.Invoke(null, false);
                            return;
                        }
        ...
        public static void GetInstallReferrer(Action<InstallReferrerResult> callback)
        {
            ...
            try
            {
                ...
                using (var prefs = context.Call<AndroidJavaObject>(
                    "getSharedPreferences", "appmachina_sdk", 0 /* MODE_PRIVATE */))
                {
                    bool alreadyCollected = prefs.Call<bool>(
                        "getBoolean", "appmachina_referrer_collected", false);
                    if (alreadyCollected)
                    {
                        context.Dispose();
            ...
            try
            {
                ...
                using (var editor = prefs.Call<AndroidJavaObject>("edit"))
                {
                    editor.Call<AndroidJavaObject>(
                        "putBoolean", "appmachina_referrer_collected", true);
                    editor.Call("apply");
                }
            }
            catch (Exception e)
            ...
            try
            {
                    ...
                    "getSharedPreferences", "appmachina_sdk", 0 /* MODE_PRIVATE */))
                {
                    string existingId = prefs.Call<string>(
                        "getString", "appmachina_install_id", (string)null);
                    ...
                    using (var editor = prefs.Call<AndroidJavaObject>("edit"))
                    {
                        editor.Call<AndroidJavaObject>("putString", "appmachina_install_id", newId);
                        editor.Call("apply");
                    }
