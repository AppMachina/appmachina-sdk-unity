...
{
    /// <summary>
    /// Disposable handle returned by <see cref="AppMachinaSDK.StartTrace"/>.
    ...
    /// thread (the actual <see cref="AppMachinaSDK.Track"/> call is
    /// dispatched to the main thread by the SDK facade).
    /// </summary>
    public sealed class AppMachinaPerformanceTrace : IDisposable
    {
        ...
        internal AppMachinaPerformanceTrace(string name)
        {
            _name = name;
        ...
        /// </summary>
        public AppMachinaPerformanceTrace SetMetadata(string key, object value)
        {
    ...
    ///    the first <see cref="AppMachinaRunner"/> Awake. Emitted exactly
    ...
    ///    <see cref="AppMachinaConfig.PerformanceFrameSamplingIntervalSec"/>
    ///    seconds while the app is foregrounded.
    /// 3. <b>Custom traces</b> — <see cref="AppMachinaSDK.Trace(string, Action)"/>
    ///    and <see cref="AppMachinaSDK.StartTrace"/> wrap a scope of work
    ...
    internal static class PerformanceModule
    {
        ...
        internal static void EmitAppStartIfNeeded()
        {
            ...
            try
            {
                AppMachinaSDK.Track(PerformanceEventName, new Dictionary<string, object>
                {
            ...
            {
                AppMachinaLogger.Warn($"PerformanceModule app-start emit failed: {e.Message}");
            }
        }

        /// <summary>
        /// Pump the periodic frame-timing sampler. Called from
        /// <see cref="AppMachinaRunner.Update"/>. Non-allocating fast-path
        ...
        private static void EmitFrameTimingSample()
        {
            ...
            try
            {
                ...
                AppMachinaSDK.Track(PerformanceEventName, props);
            }
            catch (Exception e)
            {
                AppMachinaLogger.Warn($"PerformanceModule frame-timing emit failed: {e.Message}");
            }
#else
        ...
        /// from <see cref="AppMachinaPerformanceTrace.Dispose"/>.
        /// </summary>
            ...
            string name, double elapsedMs, IDictionary<string, object> metadata)
        {
            ...
            try
            {
                AppMachinaSDK.Track(PerformanceTraceEventName, props);
            }
            catch (Exception e)
            {
                AppMachinaLogger.Warn($"PerformanceModule trace emit failed: {e.Message}");
            }
        }
    }
}
