...
namespace AppMachina.Unity.Internal
{
    ...
    /// core via <see cref="NativeBindings.appmachina_drain_batch"/> and POSTs them to the
    ...
    /// <see cref="NativeBindings.appmachina_requeue_events"/> and the drain loop stops.
    /// The periodic timer will retry on the next tick.
    /// </summary>
    internal class FlushManager
    {
        private readonly AppMachinaRunner _runner;
        private readonly uint _batchSize;
        private bool _isFlushing;
        private Coroutine _periodicCoroutine;

        internal FlushManager(AppMachinaRunner runner, uint batchSize = 20)
        {
            _runner = runner;
        ...
        /// via the Rust core's <c>appmachina_flush()</c> (which writes to the persistence
        ...
        {
            string error = NativeStringHelper.ProcessResult(NativeBindings.appmachina_flush());
            if (error != null)
                AppMachinaLogger.Warn($"Blocking flush failed: {error}");
        }
        ...
        private IEnumerator DrainAndSend()
        {
            ...
            try
            {
                string url = NativeStringHelper.ReadAndFree(NativeBindings.appmachina_events_url());
                if (string.IsNullOrEmpty(url))
                {
                    AppMachinaLogger.Warn("Flush skipped: no events URL available");
                    yield break;
                }

                string headersJson = NativeStringHelper.ReadAndFree(
                    NativeBindings.appmachina_flush_headers_json());
                var headers = ParseHeaders(headersJson);

                while (true)
                {
                    string batch = NativeStringHelper.ReadAndFree(
                        NativeBindings.appmachina_drain_batch(_batchSize));
                    ...
                    using (var request = new UnityWebRequest(url, UnityWebRequest.kHttpVerbPOST))
                    {
                        ...
                        if (!success)
                        {
                            ...
                            NativeStringHelper.ProcessResult(
                                NativeBindings.appmachina_requeue_events(batch));
                            AppMachinaLogger.Warn(
                                $"Flush failed (HTTP {request.responseCode}): {request.error}");
                            break;
                        }

                        AppMachinaLogger.Log($"Flushed batch ({bodyRaw.Length} bytes)");
                    }
                }
            }
            finally
        ...
        /// <summary>
        /// Parse the headers JSON returned by <c>appmachina_flush_headers_json()</c>.
        ///
