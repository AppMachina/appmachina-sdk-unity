...
namespace AppMachina.Unity.Internal
{
    ...
    /// to the Rust core via <see cref="NativeBindings.appmachina_update_remote_config"/>.
    ///
    ...
    internal class RemoteConfigPoller
    {
        ...
        internal RemoteConfigPoller(AppMachinaRunner runner, string baseUrl, string appId)
        {
            _runner = runner;
            // Ensure no trailing slash on the base URL
            _baseUrl = baseUrl != null ? baseUrl.TrimEnd('/') : "https://in.appmachina.com";
            _appId = appId;
        }
        ...
        private IEnumerator FetchConfig()
        {
            ...
            using (var request = UnityWebRequest.Get(url))
            {
                ...
                if (request.responseCode == 200)
                {
                    ...
                    if (!string.IsNullOrEmpty(body))
                    {
                        string error = NativeStringHelper.ProcessResult(
                            NativeBindings.appmachina_update_remote_config(body, newEtag));

                        if (error != null)
                        {
                            AppMachinaLogger.Warn($"Remote config update failed: {error}");
                        }
                        else
                        {
                            AppMachinaLogger.Log("Remote config updated");
                            ...
                            {
                                AppMachinaLogger.Warn($"OnConfigUpdated handler threw: {e.Message}");
                            }
                        }
                    }
                }
                ...
                {
                    AppMachinaLogger.Log("Remote config not modified");
                }
                else
                {
