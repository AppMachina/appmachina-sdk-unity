...
namespace AppMachina.Unity.Internal
{
    ...
    internal class NativePlatform : IAppMachinaPlatform
    {
        ...
        public string GetDebugToken()
        {
            ...
            return NativeStringHelper.ReadAndFree(NativeBindings.appmachina_get_debug_token());
        }
        ...
        {
            return NativeStringHelper.ProcessResult(
                NativeBindings.appmachina_set_super_properties(propertiesJson));
        }

        public string SetSuperPropertiesOnce(string propertiesJson)
        {
            return NativeStringHelper.ProcessResult(
                NativeBindings.appmachina_set_super_properties_once(propertiesJson));
        }

        public string UnregisterSuperProperty(string key)
        {
            return NativeStringHelper.ProcessResult(
                NativeBindings.appmachina_unregister_super_property(key));
        }

        public string ClearSuperProperties()
        {
            return NativeStringHelper.ProcessResult(NativeBindings.appmachina_clear_super_properties());
        }

        public string GetSuperPropertiesJson()
        {
            return NativeStringHelper.ReadAndFree(NativeBindings.appmachina_get_super_properties_json())
                ?? "{}";
        }
        ...
        {
            return NativeStringHelper.ProcessResult(NativeBindings.appmachina_time_event(eventName));
        }

        public ulong CancelTimedEvent(string eventName)
        {
            return NativeBindings.appmachina_cancel_timed_event(eventName);
        }
        ...
        {
            return NativeStringHelper.ProcessResult(
                NativeBindings.appmachina_set_group(groupType, groupId ?? string.Empty));
        }

        public string AddGroup(string groupType, string groupId)
        {
            return NativeStringHelper.ProcessResult(
                NativeBindings.appmachina_add_group(groupType, groupId ?? string.Empty));
        }

        public string RemoveGroup(string groupType)
        {
            return NativeStringHelper.ProcessResult(NativeBindings.appmachina_remove_group(groupType));
        }

        public string GetGroupsJson()
        {
            return NativeStringHelper.ReadAndFree(NativeBindings.appmachina_get_groups_json()) ?? "{}";
        }
        ...
        {
            return NativeStringHelper.ProcessResult(NativeBindings.appmachina_increment(key, delta));
        }

        public string Append(string key, string valueJson)
        {
            return NativeStringHelper.ProcessResult(NativeBindings.appmachina_append(key, valueJson));
        }

        public string Union(string key, string valuesJson)
        {
            return NativeStringHelper.ProcessResult(NativeBindings.appmachina_union(key, valuesJson));
        }

        public string Unset(string key)
        {
            return NativeStringHelper.ProcessResult(NativeBindings.appmachina_unset(key));
        }
        ...
        public string Reset()
        {
            return NativeStringHelper.ProcessResult(NativeBindings.appmachina_reset());
        }

        public string GetDeviceId()
        {
            return NativeStringHelper.ReadAndFree(NativeBindings.appmachina_get_device_id());
        }

        public string GetAnonymousId()
        {
            return NativeStringHelper.ReadAndFree(NativeBindings.appmachina_get_anonymous_id());
        }

        public uint GetSessionNumber()
        {
            return NativeBindings.appmachina_get_session_number();
        }

        public string GetFirstOpenTime()
        {
            return NativeStringHelper.ReadAndFree(NativeBindings.appmachina_get_first_open_time());
        }
        ...
        public string SetBeforeSend(Func<string, string> callback)
        {
            ...
            lock (_beforeSendLock)
            {
                ...
                return NativeStringHelper.ProcessResult(
                    NativeBindings.appmachina_set_before_send(_beforeSendCb, _beforeSendFreeCb));
            }
        }
        ...
        public string ClearBeforeSend()
        {
            lock (_beforeSendLock)
            {
                ...
                return NativeStringHelper.ProcessResult(NativeBindings.appmachina_clear_before_send());
            }
        }
        ...
        private static IntPtr BeforeSendTrampoline(IntPtr eventJsonPtr)
        {
            try
            {
                ...
                {
                    AppMachinaLogger.Warn($"before_send callback threw, dropping event: {ex.Message}");
                    return IntPtr.Zero;
                }
            ...
            {
                AppMachinaLogger.Warn($"before_send trampoline failed: {ex.Message}");
                return IntPtr.Zero;
            }
        }
        ...
        private static void BeforeSendFreeTrampoline(IntPtr ptr)
        {
            ...
            {
                AppMachinaLogger.Warn($"before_send free trampoline failed: {ex.Message}");
            }
        }
        ...
        public string GetFeatureFlagJson(string flagKey)
        {
            return NativeStringHelper.ReadAndFree(
                NativeBindings.appmachina_get_feature_flag(flagKey));
        }

        public int IsFeatureEnabled(string flagKey)
        {
            return NativeBindings.appmachina_is_feature_enabled(flagKey);
        }

        public string GetFeatureFlagPayloadJson(string flagKey)
        {
            return NativeStringHelper.ReadAndFree(
                NativeBindings.appmachina_get_feature_flag_payload(flagKey));
        }

        public string GetAllFlagsJson()
        {
            return NativeStringHelper.ReadAndFree(
                NativeBindings.appmachina_get_all_flags_json());
        }

        public int ReloadFeatureFlags()
        {
            return NativeBindings.appmachina_reload_feature_flags();
        }

        public string SetPersonPropertiesForFlags(string propertiesJson)
        {
            return NativeStringHelper.ProcessResult(
                NativeBindings.appmachina_set_person_properties_for_flags(propertiesJson));
        }

        public string SetFeatureFlagBootstrap(string bootstrapJson)
        {
            return NativeStringHelper.ProcessResult(
                NativeBindings.appmachina_set_feature_flag_bootstrap(bootstrapJson));
        }
    }
}
