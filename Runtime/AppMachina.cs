...
using System.Text;
using AppMachina.Unity.Internal;
using UnityEngine;
...
{
    /// <summary>
    /// Main public API for the AppMachina Unity SDK.
    ///
    /// Static singleton facade that delegates all analytics logic to the Rust core
    /// via <see cref="IAppMachinaPlatform"/>. On native targets (iOS, Android, desktop),
    ...
    /// Usage:
    /// <code>
    /// AppMachinaSDK.Initialize(new AppMachinaConfig { AppId = "your-app-id" });
    /// AppMachinaSDK.Track("button_clicked", new Dictionary&lt;string, object&gt; { ["button"] = "signup" });
    /// AppMachinaSDK.Identify("user-123");
    /// AppMachinaSDK.Flush();
    /// </code>
    ///
    /// Lifecycle is managed automatically via <see cref="AppMachinaRunner"/>:
    /// - Background: flushes queued events
    /// - Foreground: resumes periodic flush
    /// - Quit: synchronous shutdown with persistence
    /// </summary>
    public static class AppMachinaSDK
    {
        // ── Constants ────────────────────────────────────────────────────

        internal const string SdkVersion = "3.0.4";

        // ── State ────────────────────────────────────────────────────────

        private static bool _isInitialized;
        private static AppMachinaConfig _config;
        private static FlushManager _flushManager;
        private static RemoteConfigPoller _configPoller;
        private static string _userId;
        private static IAppMachinaPlatform _platform;
        ...
        /// the <see cref="AppMachinaConfig"/> passed to <see cref="Initialize"/>.
        /// </summary>
        /// <remarks>
        ...
        /// </summary>
        internal static AppMachinaConfig Config => _config;

        // ── Initialization ───────────────────────────────────────────────

        /// <summary>
        /// Initialize the AppMachina SDK. Must be called once before any tracking,
        ...
        ///
        /// Throws <see cref="ArgumentException"/> if <see cref="AppMachinaConfig.AppId"/>
        /// is null or empty.
        /// </summary>
        /// <param name="config">SDK configuration. Only <c>AppId</c> is required.</param>
        public static void Initialize(AppMachinaConfig config)
        {
            if (_isInitialized)
            {
                AppMachinaLogger.Warn("AppMachina SDK already initialized");
                return;
            }
            ...
            _config = config;
            AppMachinaLogger.Enabled = config.EnableDebug;
            ...
            _platform = AppMachinaPlatformFactory.Create();
            ...
            var configDict = new Dictionary<string, object>
            {
                ...
                // Tier 5/6 — read by AppMachinaWebGL.jslib to install
                ...
                // hooks check the AppMachinaConfig fields directly.
            ...
            var runner = AppMachinaRunner.Instance;
...
#if UNITY_WEBGL && !UNITY_EDITOR
            // jslib handles config polling — see AppMachinaWebGL_StartConfigPolling
#else
            string baseUrl = !string.IsNullOrEmpty(config.BaseUrl)
                ? config.BaseUrl
                : "https://in.appmachina.com";
            ...
            // core. DeepLinksModule uses NativeBindings (P/Invoke to appmachina_core)
            ...
            if (config.FeatureFlagBootstrap != null)
            {
                ...
                if (bootstrapErr != null)
                    AppMachinaLogger.Warn($"FeatureFlagBootstrap apply failed: {bootstrapErr}");
            }
            ...
            Track("appmachina_init_timing", new Dictionary<string, object>
            {
                ["duration_ms"] = initDurationMs
            });

            AppMachinaLogger.Log($"AppMachina SDK initialized in {initDurationMs}ms (appId={config.AppId}, env={config.Environment})");
        }
            ...
            Dictionary<string, object> properties, bool setOnce)
        {
            ...
            AppMachinaRunner.Instance.StartCoroutine(PostUserProperties(url, body));
        }
        ...
        public static void Reset()
        {
            ...
            _userId = null;
            AppMachinaLogger.Log("User state reset");
        }
        ...
        /// safely guard with <c>if (AppMachinaSDK.IsFeatureEnabled("x"))</c>.
        ...
        public static Dictionary<string, object> GetAllFlags()
        {
            ...
            {
                AppMachinaLogger.Warn($"GetAllFlags: failed to parse JSON: {e.Message}");
                return new Dictionary<string, object>();
            }
        }
        ...
        /// </summary>
        public static void SetFeatureFlagBootstrap(AppMachinaFeatureFlagBootstrap bootstrap)
        {
        ...
        /// <summary>Serialize an <see cref="AppMachinaFeatureFlagBootstrap"/>
        /// to the wire JSON shape expected by the Rust core.</summary>
        private static string SerializeBootstrap(AppMachinaFeatureFlagBootstrap b)
        {
        ...
        private static void NotifyFeatureFlagListeners()
        {
            ...
            {
                AppMachinaLogger.Warn($"NotifyFeatureFlagListeners: GetAllFlags failed: {e.Message}");
                return;
            }
            foreach (var cb in snapshot)
            {
                ...
                {
                    AppMachinaLogger.Warn($"OnFeatureFlags listener threw: {e.Message}");
                }
            }
        }
        ...
        public static void Shutdown()
        {
            ...
            // hooks and tears them down via AppMachinaWebGL_Shutdown.
            ...
            AppMachinaLogger.Log("AppMachina SDK shut down");
        }
        ...
        /// disabled <see cref="AppMachinaConfig.AutoTrackExceptions"/> or
        ...
        ///
        /// <code>
        /// AppMachinaSDK.Trace("load_user_profile", () => {
        ///     // ... work ...
        /// });
        /// </code>
        ///
        ...
        public static void Trace(string name, Action body)
        {
            ...
            var trace = new AppMachinaPerformanceTrace(name);
            try
            {
                body();
            }
        ...
        ///
        /// <code>
        /// using (var trace = AppMachinaSDK.StartTrace("checkout_flow")) {
        ///     trace.SetMetadata("step_count", 4);
        ///     // ... multi-step work ...
        /// }
        /// </code>
        /// </summary>
        public static AppMachinaPerformanceTrace StartTrace(string name)
        {
            if (!CheckInitialized("StartTrace")) return new AppMachinaPerformanceTrace(name ?? "unknown");
            if (string.IsNullOrEmpty(name))
            {
                RaiseError("StartTrace", "name must not be null or empty");
                name = "unknown";
            }
            return new AppMachinaPerformanceTrace(name);
        }
        ...
        ///
        /// The callback receives the resulting <see cref="AppMachinaATTStatus"/>.
        /// On non-iOS platforms, the callback receives <see cref="AppMachinaATTStatus.NotDetermined"/>.
        /// </summary>
        /// <param name="callback">Called with the ATT status after the user responds.</param>
        public static void RequestTrackingPermission(Action<AppMachinaATTStatus> callback = null)
        {
            ...
            ATTModule.RequestTracking(status =>
            {
                ...
                bool advertisingAllowed = status == AppMachinaATTStatus.Authorized;
                SetConsent(advertising: advertisingAllowed);

                AppMachinaLogger.Log(
        ...
        private const string PrefDeeplinkId = "appmachina_attribution_deeplink_id";
        private const string PrefGclid = "appmachina_attribution_gclid";
        private const string PrefFbclid = "appmachina_attribution_fbclid";
        private const string PrefFbc = "appmachina_attribution_fbc";
        private const string PrefTtclid = "appmachina_attribution_ttclid";
        private const string PrefMsclkid = "appmachina_attribution_msclkid";
        ...
        public static void ShowDebugOverlay()
        {
            ...
            AppMachinaLogger.Log("Debug overlay shown");
        }
        ...
        public static void HideDebugOverlay()
        {
            if (_debugOverlay != null)
            {
                UnityEngine.Object.Destroy(_debugOverlay);
                _debugOverlay = null;
                AppMachinaLogger.Log("Debug overlay hidden");
            }
        }
        ...
        // ── Internal Lifecycle Callbacks (called by AppMachinaRunner) ─────────

        internal static void OnBackgrounded()
        {
            if (!_isInitialized) return;
            AppMachinaLogger.Log("App backgrounded, flushing...");
            ...
            if (_config != null && _config.AutoCaptureLifecycle)
            {
                try
                {
                    Track(StandardEvents.AppMachinaAppBackground);
                }
                catch (Exception e)
                {
                    AppMachinaLogger.Warn($"Failed to emit $app_background: {e.Message}");
                }
            }
        ...
        {
            if (!_isInitialized) return;
            AppMachinaLogger.Log("App foregrounded");
        ...
        {
            if (!_isInitialized) return;
            AppMachinaLogger.Log("Network reconnected, flushing...");
#if UNITY_WEBGL && !UNITY_EDITOR
            _platform?.Flush();
#else
            _flushManager?.FlushNow();
        ...
        /// </summary>
        private const string LastAppVersionKey = "appmachina_last_app_version";

        /// <summary>
        /// Cold launch (called from <see cref="AppMachinaRunner.Start"/>).
        ...
        /// <see cref="AppMachinaConfig.AutoCaptureLifecycle"/> is false.
        /// </summary>
        internal static void OnLifecycleColdLaunch()
        {
            ...
            // PlayerPrefs("appmachina_first_launch_tracked") via InstallEventGate;
            ...
            if (isFirstLaunch)
            {
                Track(StandardEvents.AppMachinaFirstOpen, new Dictionary<string, object>
                {
            ...
            if (!isFirstLaunch
                ...
                && previous != current)
            {
                Track(StandardEvents.AppMachinaAppUpdate, new Dictionary<string, object>
                {
        ...
        internal static void OnLifecycleTerminate()
        {
            ...
            try
            {
                Track(StandardEvents.AppMachinaAppTerminate);
            }
            catch (Exception e)
            {
                AppMachinaLogger.Warn($"Failed to emit $app_terminate: {e.Message}");
            }
        }
        ...
        private static void EmitAppOpen(bool coldLaunch)
        {
            try
            {
                ...
                Track(StandardEvents.AppMachinaAppOpen, props);
            }
            catch (Exception e)
            {
                AppMachinaLogger.Warn($"Failed to emit $app_open: {e.Message}");
            }
        }
        ...
        private static bool CheckInitialized(string method)
        {
            ...
            RaiseError(method, "AppMachina SDK not initialized. Call AppMachinaSDK.Initialize() first.");
            return false;
        }

        private static void RaiseError(string method, string message)
        {
            AppMachinaLogger.Error($"[{method}] {message}");
            try
            {
            ...
            catch (Exception e)
            {
                ...
                AppMachinaLogger.Warn($"OnError handler threw: {e.Message}");
            }
        }
        ...
        /// </summary>
        private static void TrackAttributionSignals(AppMachinaConfig config)
        {
            ...
            {
                AppMachinaLogger.Warn($"AdServices token collection failed: {e.Message}");
            }
#endif
            ...
            {
                AppMachinaLogger.Warn($"WebGL CAPI/attribution capture failed: {e.Message}");
            }
#endif
            ...
            {
                AppMachinaLogger.Warn($"Clipboard attribution check failed: {e.Message}");
            }
#endif
        ...
        private static void OnRemoteConfigUpdated(string configJson)
        {
            ...
            {
                AppMachinaLogger.Warn($"SKAN auto-config failed: {e.Message}");
            }
        }
#endif
        ...
        private static void InitAndroidModules()
        {
            ...
            AndroidModule.GetInstallReferrer(result =>
            {
                if (result != null && _isInitialized)
                {
                    ...
                    if (_config != null && _config.EnableDebug)
                    {
                        AppMachinaLogger.Log($"Install referrer tracked: {result.RawReferrer}");
                    }
                }
            });
        }
        ...
        {
            AppMachinaLogger.Log("WebGL platform initialized (jslib manages lifecycle, HTTP, and SPA deep link tracking)");
        }
#endif
    }
}
