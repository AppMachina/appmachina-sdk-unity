...
{
    /// <summary>
    /// RevenueCat integration for the AppMachina Unity SDK.
    ///
    /// Provides static methods to forward purchase/subscription events to AppMachina
    ...
    public static class RevenueCatIntegration
    {
            ...
            string store = null)
        {
            try
            {
                ...
                AppMachinaSDK.Track("purchase_success", new Dictionary<string, object>
                {
            ...
            {
                AppMachinaLogger.Warn($"RevenueCatIntegration.TrackPurchase error: {e.Message}");
            }
        }

        /// <summary>
        /// Sync user attributes (subscriber status, original app user ID) to AppMachina.
        /// Calls <see cref="AppMachinaSDK.SetUserProperties"/> with <c>is_subscriber</c> and
        ...
        public static void SyncAttributes(bool isSubscriber, string originalAppUserId = null)
        {
            try
            {
                ...
                AppMachinaSDK.SetUserProperties(props);
            }
            catch (Exception e)
            {
                AppMachinaLogger.Warn($"RevenueCatIntegration.SyncAttributes error: {e.Message}");
            }
        }
            ...
            string originalAppUserId = null)
        {
            try
            {
                ...
                if (_isInitialized)
                {
                    foreach (string productId in current)
                    {
                        ...
                        {
                            AppMachinaSDK.Track("subscription_start", new Dictionary<string, object>
                            {
                                ["product_id"] = productId,
                                ["source"] = "revenuecat"
                            });
            ...
            {
                AppMachinaLogger.Warn($"RevenueCatIntegration.OnCustomerInfoUpdated error: {e.Message}");
            }
        }
