...
{
    /// <summary>
    /// Superwall integration for the AppMachina Unity SDK.
    ///
    /// Provides static methods to forward paywall lifecycle events to AppMachina
    /// without requiring a hard dependency on the Superwall Unity SDK.
    ///
    /// Each method calls <see cref="AppMachinaSDK.Track"/> with appropriate event names:
    /// <c>paywall_show</c>, <c>paywall_dismiss</c>, <c>paywall_purchase</c>, <c>paywall_skip</c>.
    /// </summary>
    public static class SuperwallIntegration
    {
        /// <summary>
        /// Track a generic Superwall event. Forwards to <see cref="AppMachinaSDK.Track"/>
        ...
        public static void OnEvent(string eventName, Dictionary<string, object> properties = null)
        {
            ...
            try
            {
                AppMachinaSDK.Track(eventName, properties);
            }
            catch (System.Exception e)
            {
                AppMachinaLogger.Warn($"SuperwallIntegration.OnEvent error: {e.Message}");
            }
        }
            ...
            string variantId = null)
        {
            try
            {
                ...
                AppMachinaSDK.Track("paywall_show", props);
            }
            catch (System.Exception e)
            {
                AppMachinaLogger.Warn($"SuperwallIntegration.TrackPresentation error: {e.Message}");
            }
        }
        ...
        public static void TrackDismiss(string paywallId)
        {
            try
            {
                AppMachinaSDK.Track("paywall_dismiss", new Dictionary<string, object>
                {
                    ["paywall_id"] = paywallId ?? "unknown"
                });
            }
            ...
            {
                AppMachinaLogger.Warn($"SuperwallIntegration.TrackDismiss error: {e.Message}");
            }
        }
            ...
            string currency = null)
        {
            try
            {
                ...
                AppMachinaSDK.Track("paywall_purchase", props);
            }
            catch (System.Exception e)
            {
                AppMachinaLogger.Warn($"SuperwallIntegration.TrackPurchase error: {e.Message}");
            }
        }
        ...
        public static void TrackSkip(string paywallId, string reason)
        {
            try
            {
                AppMachinaSDK.Track("paywall_skip", new Dictionary<string, object>
                {
                    ["paywall_id"] = paywallId ?? "unknown",
            ...
            {
                AppMachinaLogger.Warn($"SuperwallIntegration.TrackSkip error: {e.Message}");
            }
        }

        /// <summary>
        /// Get user attributes for Superwall's user attribute API.
        /// Returns a dictionary with <c>appmachina_session_id</c> and <c>appmachina_user_id</c>.
        ///
        /// Usage:
        /// <code>
        ...
        public static Dictionary<string, object> UserAttributes()
        {
            ...
            try
            {
                string sessionId = AppMachinaSDK.SessionId;
                if (!string.IsNullOrEmpty(sessionId))
                    attrs["appmachina_session_id"] = sessionId;

                string userId = AppMachinaSDK.UserId;
                if (!string.IsNullOrEmpty(userId))
                    attrs["appmachina_user_id"] = userId;
            }
            catch (System.Exception e)
            {
                AppMachinaLogger.Warn($"SuperwallIntegration.UserAttributes error: {e.Message}");
            }
