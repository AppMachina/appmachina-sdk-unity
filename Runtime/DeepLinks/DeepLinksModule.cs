...
namespace AppMachina.Unity
{
    ...
    /// the main <c>AppMachinaSDK</c> class. Consumer code registers listeners via
    ...
    public static class DeepLinksModule
    {
        ...
        // ── Internal Init / Teardown (called by AppMachinaSDK class) ─────────
        ...
        /// <summary>
        /// Initialize the deep links module. Called by the main AppMachinaSDK class during init.
        /// </summary>
        /// <param name="enabled">
        /// Whether to auto-track <c>deep_link_opened</c> events.
        /// Controlled by <c>AppMachinaConfig.AutoTrackDeepLinks</c>.
        /// </param>
        ...
        internal static void Init(bool enabled, bool enableDebug)
        {
            ...
            if (s_enableDebug)
            {
                Debug.Log("[AppMachina] DeepLinksModule initialized" +
                          (s_enabled ? " (auto-tracking enabled)" : " (auto-tracking disabled)"));
            }
        }

        /// <summary>
        /// Tear down the deep links module. Called by the main AppMachinaSDK class during shutdown.
        /// </summary>
        internal static void Teardown()
        {
            ...
            if (s_enableDebug)
            {
                Debug.Log("[AppMachina] DeepLinksModule torn down");
            }
        }
        ...
        private static void OnDeepLinkActivated(string url)
        {
            ...
            catch (Exception e)
            {
                if (s_enableDebug)
                {
                    Debug.LogWarning($"[AppMachina] Failed to parse deep link URL: {url} — {e.Message}");
                }
                return;
            }
            ...
            catch (Exception e)
            {
                if (s_enableDebug)
                {
                    Debug.LogWarning($"[AppMachina] Exception in OnDeepLinkReceived listener: {e.Message}");
                }
            }
        }
        ...
        /// <see cref="AppMachinaSDK.SetAttributionData"/> so they are included
        ...
        private static void PersistClickIds(AttributionData attr)
        {
            ...
            AppMachinaSDK.SetAttributionData(
                deeplinkId: AppMachinaSDK.DeeplinkId,
                gclid: !string.IsNullOrEmpty(attr.Gclid) ? attr.Gclid : AppMachinaSDK.Gclid,
                fbclid: !string.IsNullOrEmpty(attr.Fbclid) ? attr.Fbclid : AppMachinaSDK.Fbclid,
                ttclid: !string.IsNullOrEmpty(attr.Ttclid) ? attr.Ttclid : AppMachinaSDK.Ttclid,
                msclkid: !string.IsNullOrEmpty(attr.Msclkid) ? attr.Msclkid : AppMachinaSDK.Msclkid
            );

            if (s_enableDebug)
            {
                Debug.Log("[AppMachina] DeepLinksModule persisted click IDs from deep link URL");
            }
        }
