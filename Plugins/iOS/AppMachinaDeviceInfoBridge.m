...
//  appmachina_devinfo_free() to release the buffer. NULL is mapped to a C#
...
static const char *appmachina_devinfo_strdup_or_null(NSString *value) {
    if (value == nil) {
        return NULL;
    }
...
const char *appmachina_ios_get_os_version(void) {
    @try {
        NSString *version = [UIDevice currentDevice].systemVersion;
        return appmachina_devinfo_strdup_or_null(version);
    }
...
const char *appmachina_ios_get_device_model(void) {
    @try {
        ...
        return appmachina_devinfo_strdup_or_null(model);
    }
    @catch (NSException *exception) {
        return NULL;
    }
...
const char *appmachina_ios_get_app_version(void) {
    @try {
...
const char *appmachina_ios_get_build_number(void) {
    @try {
        NSString *build = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"CFBundleVersion"];
        return appmachina_devinfo_strdup_or_null(build);
    }
    @catch (NSException *exception) {
        return NULL;
    }
...
/// Free a string previously returned by one of the appmachina_ios_get_*
/// functions. Safe to call with NULL (no-op).
void appmachina_devinfo_free(const char *ptr) {
    if (ptr != NULL) {
        free((void *)ptr);
    }
}
