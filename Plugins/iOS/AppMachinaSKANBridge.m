...
#define APPMACHINA_HAS_SKAN 1
#else
#define APPMACHINA_HAS_SKAN 0
#endif

/// Check if SKAdNetwork is supported on this device (iOS 14.0+).
bool appmachina_skan_is_supported(void) {
#if APPMACHINA_HAS_SKAN
    if (@available(iOS 14, *)) {
        return true;
    }
#endif
...
const char* appmachina_skan_get_version(void) {
#if APPMACHINA_HAS_SKAN
...
void appmachina_skan_register(void) {
#if APPMACHINA_HAS_SKAN
    if (@available(iOS 15.4, *)) {
        [SKAdNetwork updatePostbackConversionValue:0
                                 completionHandler:^(NSError * _Nullable error) {
            if (error) {
                NSLog(@"[AppMachina] SKAdNetwork.updatePostbackConversionValue(0) failed: %@", error.localizedDescription);
            }
        }];
        return;
    }
...
void appmachina_skan_update_conversion_value(int fineValue) {
#if APPMACHINA_HAS_SKAN
    if (@available(iOS 15.4, *)) {
        [SKAdNetwork updatePostbackConversionValue:fineValue
                                 completionHandler:^(NSError * _Nullable error) {
            if (error) {
                NSLog(@"[AppMachina] SKAdNetwork.updatePostbackConversionValue(%d) failed: %@", fineValue, error.localizedDescription);
            }
        }];
        return;
    }
...
void appmachina_skan_update_postback(int fineValue, const char* coarseValue, bool lockWindow) {
#if APPMACHINA_HAS_SKAN
    if (@available(iOS 16.1, *)) {
        ...
        [SKAdNetwork updatePostbackConversionValue:fineValue
                                ...
                                completionHandler:^(NSError * _Nullable error) {
            if (error) {
                NSLog(@"[AppMachina] SKAdNetwork.updatePostbackConversionValue(%d, %@, %d) failed: %@",
