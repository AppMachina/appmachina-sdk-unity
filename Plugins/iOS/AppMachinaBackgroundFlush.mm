//
//  AppMachinaBackgroundFlush.mm
//  AppMachina Unity SDK
//
//  Objective-C++ bridge for iOS background flush via BGAppRefreshTask.
//  Registers a BGAppRefreshTask with identifier "com.appmachina.sdk.background-flush"
...
//  In UnityAppController+AppMachina.mm or your custom AppDelegate,
//  call appmachina_background_flush_register() in
//  application:didFinishLaunchingWithOptions:.
//
//  Setup: Add "com.appmachina.sdk.background-flush" to Info.plist under
...
#define APPMACHINA_HAS_BGTASKS 1
#else
#define APPMACHINA_HAS_BGTASKS 0
#endif

static NSString *const kTaskIdentifier = @"com.appmachina.sdk.background-flush";
...
static void appmachina_bg_schedule_next_flush(void);
static void appmachina_bg_handle_task(id task);

#pragma mark - Internal helpers

static void appmachina_bg_schedule_next_flush(void) {
#if APPMACHINA_HAS_BGTASKS
    if (@available(iOS 13.0, *)) {
        ...
        if (error) {
            NSLog(@"[AppMachina] BGAppRefreshTask scheduling failed: %@", error.localizedDescription);
        }
    }
#endif
}
...
static void appmachina_bg_handle_task(BGTask *task) {
    // Schedule the next execution before starting work
    appmachina_bg_schedule_next_flush();
    ...
    // appmachina_background_flush_completed() when done.
    atomic_store(&sFlushCompleted, false);
    UnitySendMessage("_AppMachinaBackgroundFlush", "OnBackgroundFlush", "");
...
extern "C" void appmachina_background_flush_register(void) {
#if APPMACHINA_HAS_BGTASKS
    if (@available(iOS 13.0, *)) {
        sIsRegistered = [BGTaskScheduler.sharedScheduler
            ...
            launchHandler:^(BGTask * _Nonnull task) {
                appmachina_bg_handle_task(task);
            }];
        if (!sIsRegistered) {
            NSLog(@"[AppMachina] BGTaskScheduler.register failed -- ensure "
                  @"appmachina_background_flush_register() is called in "
                  @"application:didFinishLaunchingWithOptions: and "
                  @"com.appmachina.sdk.background-flush is in Info.plist "
                  @"BGTaskSchedulerPermittedIdentifiers");
        }
    }
#endif
...
extern "C" bool appmachina_background_flush_enable(void) {
#if APPMACHINA_HAS_BGTASKS
    if (@available(iOS 13.0, *)) {
        if (!sIsRegistered) {
            appmachina_background_flush_register();
        }
        if (!sIsRegistered) {
            return false;
        }
...
extern "C" void appmachina_background_flush_completed(void) {
    atomic_store(&sFlushCompleted, true);
}

/// Disable periodic background flush. Cancels any pending task requests.
extern "C" void appmachina_background_flush_disable(void) {
#if APPMACHINA_HAS_BGTASKS
    if (@available(iOS 13.0, *)) {
