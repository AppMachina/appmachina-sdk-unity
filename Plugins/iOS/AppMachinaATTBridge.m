...
#define APPMACHINA_HAS_ATT 1
#else
#define APPMACHINA_HAS_ATT 0
#endif

#if __has_include(<AdSupport/ASIdentifierManager.h>)
#import <AdSupport/ASIdentifierManager.h>
#define APPMACHINA_HAS_ADSUPPORT 1
#else
#define APPMACHINA_HAS_ADSUPPORT 0
#endif
...
typedef void (*AppMachinaATTCallback)(int status);

/// Check if ATT is available (iOS 14.0+).
bool appmachina_att_is_available(void) {
#if APPMACHINA_HAS_ATT
    if (@available(iOS 14, *)) {
        return true;
    }
#endif
...
int appmachina_att_get_status(void) {
#if APPMACHINA_HAS_ATT
    if (@available(iOS 14, *)) {
...
void appmachina_att_request_tracking(AppMachinaATTCallback callback) {
#if APPMACHINA_HAS_ATT
    if (@available(iOS 14, *)) {
...
const char* appmachina_att_get_idfa(void) {
#if APPMACHINA_HAS_ATT && APPMACHINA_HAS_ADSUPPORT
    if (@available(iOS 14, *)) {
...
/// Returns a strdup'd C string. Unity marshals and frees it.
const char* appmachina_att_get_idfv(void) {
