...
{
    /// <summary>
    /// Android post-build processor for the AppMachina SDK.
    ...
    /// </summary>
    public class AppMachinaAndroidPostBuildProcessor : IPostGenerateGradleAndroidProject
    {
        /// <summary>
        ...
        public void OnPostGenerateGradleAndroidProject(string path)
        {
            ...
            {
                Debug.LogWarning(
                    $"[AppMachina] AndroidManifest.xml not found at expected path: {manifestPath}");
                return;
            }
            ...
            {
                Debug.LogWarning(
                    "[AppMachina] Could not find main Activity in AndroidManifest.xml. " +
            ...
            foreach (var filter in settings.intentFilters)
            {
                ...
                {
                    Debug.LogWarning(
                        "[AppMachina] Intent filter missing required 'scheme' field. Skipping.");
                    continue;
                }
            ...
            if (added > 0)
            {
                ...
                Debug.Log($"[AppMachina] Added {added} intent filter(s) to AndroidManifest.xml.");
            }
            else
            {
                Debug.Log("[AppMachina] All intent filters already present. No changes needed.");
            }
        }
